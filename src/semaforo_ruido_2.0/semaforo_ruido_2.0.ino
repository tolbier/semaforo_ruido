...
int rele[]={10,5,4,A5};       //Define los pins donde estan conectados los 4 reles
...
void loop() {
  ...
  //float amplitudeAvg=   getAmplitudeReading();
  float amplitudeReading =  getAmplitudeReadingMaxOnLastInterval();
  //Si ha pasado el tiempo delay_between_last_activation, evaluamos la AmplitudMedia
  evaluateAmplitude(amplitudeReading);

  //mostramos resultados en Serial
  printThresholds();
  printLevels();
  printSound(amplitudeReading);
